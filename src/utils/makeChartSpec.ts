...
const initChart: VisualizationSpec = {
...
export function makeVegaChartSpec(data: number[] = []): VisualizationSpec {

  const chart: VisualizationSpec = { ...initChart };
  if (Array.isArray(chart.data) && chart.data[0] && 'values' in chart.data[0]) {
    (chart.data[0] as { values: any[] }).values = data.map((d) => ({ u: d }));
  }
